...
FULL_GDN_CASES: tuple[Stage2Case, ...] = (
    Stage2Case("F1", 64, 8, 8, 1024, 128, 128, 64, "fp16", "fp16"),
    Stage2Case("F2", 32, 16, 16, 2048, 128, 128, 64, "bf16", "bf16"),
    Stage2Case("F3", 16, 32, 32, 4096, 128, 128, 128, "fp16", "fp32"),
    Stage2Case("F4", 8, 32, 32, 8192, 128, 128, 128, "bf16", "bf16"),
    Stage2Case("F5", 128, 4, 4, 1024, 128, 128, 64, "fp16", "fp16"),
    Stage2Case("F6", 64, 8, 8, 2048, 128, 128, 64, "bf16", "fp32"),
    Stage2Case("F7", 32, 16, 16, 4096, 128, 128, 128, "fp16", "fp16"),
    Stage2Case("F8", 16, 32, 32, 8192, 128, 128, 128, "bf16", "bf16"),
    Stage2Case("F9", 64, 8, 8, 4096, 128, 128, 128, "fp16", "fp16"),
    Stage2Case("F10", 32, 16, 16, 8192, 128, 128, 128, "bf16", "bf16"),
    Stage2Case("F11", 16, 32, 32, 16384, 128, 128, 64, "fp16", "fp32"),
    Stage2Case("F12", 8, 32, 32, 32768, 128, 128, 64, "bf16", "bf16"),
    Stage2Case("F13", 64, 8, 8, 1024, 128, 128, 64, "fp16", "fp16"),
    Stage2Case("F14", 32, 16, 16, 2048, 128, 128, 64, "bf16", "bf16"),
    Stage2Case("F15", 16, 32, 32, 4096, 128, 128, 128, "fp16", "fp32"),
    Stage2Case("F16", 8, 32, 32, 8192, 128, 128, 128, "bf16", "bf16"),
    Stage2Case("F17", 64, 8, 8, 2048, 128, 128, 64, "bf16", "bf16"),
    Stage2Case("F18", 32, 16, 16, 4096, 128, 128, 128, "fp16", "fp16"),
    Stage2Case("L1", 1, 32, 32, 65536, 128, 128, 64, "bf16", "bf16", 64),
    Stage2Case("L2", 1, 16, 16, 65536, 128, 128, 64, "fp16", "fp16", 33),
    Stage2Case("L3", 1, 8, 8, 131072, 128, 128, 64, "bf16", "bf16", 333),
    Stage2Case("L4", 1, 4, 4, 262144, 128, 128, 64, "fp16", "fp32", 567),
    Stage2Case("L5", 1, 16, 16, 32768, 128, 128, 64, "bf16", "bf16", 7),
    Stage2Case("L6", 1, 8, 8, 65536, 128, 128, 64, "fp16", "fp16", 25),
    Stage2Case("L7", 1, 16, 32, 16384, 128, 256, 64, "fp16", "fp32", 128),
    Stage2Case("L8", 1, 21, 63, 16384, 128, 256, 64, "bf16", "fp32", 2),
    Stage2Case("L9", 1, 8, 32, 65536, 128, 256, 128, "bf16", "fp32", 172),
    Stage2Case("L10", 1, 16, 32, 65536, 128, 128, 64, "fp16", "fp32", 668),
    Stage2Case("L11", 1, 4, 32, 65536, 128, 128, 128, "bf16", "fp32", 17),
    Stage2Case("L12", 1, 2, 64, 262144, 128, 256, 64, "fp16", "fp32", 32),
    Stage2Case("F19", 1, 16, 32, 4096, 128, 256, 64, "fp16", "fp32"),
    Stage2Case("F20", 16, 21, 63, 2048, 128, 256, 64, "bf16", "fp32"),
    Stage2Case("F21", 711, 4, 32, 196, 128, 128, 128, "fp16", "fp32"),
    Stage2Case("F22", 176, 2, 64, 24, 128, 256, 64, "bf16", "fp32"),
)
...
def make_inputs(case: Stage2Case, device: str, seed: int, input_scale: float):
...
def build_tasks(case: Stage2Case, cu_seqlens: list[int] | None, chunk_indices: list[int] | None) -> list[TaskRange]:
...
def run_case(case: Stage2Case, args) -> list[str]:
    ...
    with torch.no_grad():
        ...
        outputs = fla_ascendc.prepare_wy_repr_bwd_stage2_debug(
        ...
        for task_idx, task in enumerate(tasks):
            ...
            compare_with_ct(
                f"{case.name}.task{task_idx}.D",
                debug_d[task_idx, :, :cur, :cur],
                expect_d[task.batch, :, task.begin:task.end, :cur],
                failures,
                args.verbose_ct,
            )
...
def select_cases(args) -> list[Stage2Case]:
    if args.cases:
...
def run_cases(cases: Iterable[Stage2Case], args) -> int:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="prepare_wy_repr_bwd stage2 D golden precision test")
