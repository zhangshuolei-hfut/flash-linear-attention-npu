...
def selected_cases() -> list[PerfCase]:
    wanted = os.environ.get("PREPARE_WY_STAGE2_PERF_CASES", "").strip()
    if not wanted:
...
def main() -> int:
    device = os.environ.get("PREPARE_WY_STAGE2_PERF_DEVICE", "npu")
    seed = int(os.environ.get("PREPARE_WY_STAGE2_PERF_SEED", "2026"))
    warmup = int(os.environ.get("PREPARE_WY_STAGE2_PERF_WARMUP", "0"))
    repeat = int(os.environ.get("PREPARE_WY_STAGE2_PERF_REPEAT", "1"))
