...
- 正式验收比较标准使用用户提供 golden 的 `ct.dual(actual.cpu(), cpu_high, cpu_normal, dtype=ktype)`，不要另写阈值、bad-count、`ct.single` 或自定义误差规则。后续测 `prepare_wy_repr_bwd` 精度默认跑用户提供 JSON 中的全量 42 个 case，包括原 JSON 中 `enabled=false` 的用例；只在定位单点问题时才使用 `--case-index`、`--case`、`--limit` 或 `--enabled-only` 缩小范围，不能把子集结果当作正式全量验收。CPU golden 可能很慢，尤其大 GVA case，不能把 CPU golden 或 `ct.dual` 长时间运行误判为 kernel 卡死。
- 用户提供 golden 的 fused 验证脚本 `test_prepare_wy_repr_bwd_fused.py` 已集成输入构造、CPU normal/high precision golden 生成、per-case 缓存读写、fused 调用和 `ct.dual` 比对；默认 `--isolate-cases` 会在选择多个 case 时逐 case 拉起子进程，避免大 case 显存/内存保活影响后续 case。默认目录为 `prepare_wy_repr_bwd_golden/prepare_wy_repr_bwd/golden_cache`，文件名包含 case index/name/seed/case signature。生成或补齐缓存使用 `python3 -u test_prepare_wy_repr_bwd_fused.py --golden-only --golden-cache-mode auto`；正式精度验证优先使用 `python3 -u test_prepare_wy_repr_bwd_fused.py --golden-cache-mode load --device <visible_device>`，必须全部 42 case 通过。若缓存缺失但允许现算，使用默认 `auto`；若修改 JSON、输入构造、seed 或 golden 公式，旧缓存自动失效或必须用 `--golden-cache-mode refresh` 重建。缓存必须同时保存 `cpu_normal` 和 `cpu_high` 四输出，并校验 seed、case signature 和 output names；全量 42 case 缓存体量约 174 GiB，应按 case 分片保存，不要打成单个大文件。
- 当前 A5 服务器已保存完整全量 golden cache：golden 根目录 `/home/npu_user7/zhangshuolei/prepare_wy_repr_bwd_golden/prepare_wy_repr_bwd`，缓存目录 `/home/npu_user7/zhangshuolei/prepare_wy_repr_bwd_golden/prepare_wy_repr_bwd/golden_cache`，生成日志 `/home/npu_user7/zhangshuolei/prepare_wy_repr_bwd_golden/logs/cache_20260723_223956/cache_all.log`，结果为 42 个 `case_*.pt`、约 174 GiB、`CACHE_ALL_DONE fail=0`。A2 服务器同步了同名 golden 根目录 `/workspace/zs/prepare_wy_repr_bwd_golden/prepare_wy_repr_bwd`，但当前未保存完整 174 GiB cache；如需在 A2 用 `--golden-cache-mode load` 做全量精度，需要先从 A5 同步 `golden_cache` 或重新在 A2 生成缓存。
