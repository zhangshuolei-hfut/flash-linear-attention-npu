...
- `prepare_wy_repr_bwd` Cube 侧 stage2 首版只复用现有 L1 scratch ping/pong，不新增 `A/DA4/DA5` resident：`DA5=DA4@A.T` 时 scratch 分别承接 `DA4` 与 `A.T`，`DA6_T=DA5.T@A` 时 scratch 分别承接 `DA5.T` 与 `A`。`A.T/A` 的 L1 packed layout 不假设兼容，因此当前阶段可以从 GM 分别搬入；若后续要做 `A` resident 或同一块 L1 同时作为 row/column view，必须先说明 layout 兼容性和事件生命周期。stage2 内 `DA5` 由 Fixpipe 写入 GM 后立即被同一个 Cube 用 MTE2 读回做 `DA6_T`，必须在 `copyL0CToGm(DA5)` 后使用成对 `SetFlag/WaitFlag<HardEvent::FIX_MTE2>`，不能只依赖 `FIX_M`，否则 `DA6_T` 可能读到全零旧值。
