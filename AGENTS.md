...
- `prepare_wy_repr_bwd` Cube 侧当前已实现 `K/DW/DU` 三类独立 L1 resident：`K` 按 HK/GVA group 驻留复用，供 `KKT` 和 stage3 `Dkb=D.T@K` 使用；每个 workspace slot 需要记录当次 head 对应的 `kResidentSlotForSlot_[slot]`，不得用 `hv & 1` 推导。`DW` 在 stage0 Dkbg 搬入并延续到 stage1 DA1，`DU` 在 stage0 Dvb 搬入并延续到 stage1 DA2。`DW/DU` resident 生命周期按 workspace `curSlot_` 管理，释放点分别在 stage1 对应 L1->L0 搬运发出后，不能在 stage0 提前释放。
...
- stage 级精度验收必须导出该 stage 的中间产物并写专门 golden 逐项比对；不能只用最终 `dk/dv/dbeta/dg` 与 `da + full` 等价来判定某个 stage 内部正确。stage0/stage1/stage2 已经分别保存为独立分支或提交，后续默认不再维护旧 stage debug 输出；当前最新验证入口是 `prepare_wy_repr_bwd_stage3_debug`，只导出 `Dkb` 和 `DK`，二者 shape 均为 `[taskNum, HV, chunkSize, K]`，只比较有效 `[curChunkSize, K]` 区域。
...
- stage debug 默认验证不跑完整 F1-F22、L1-L12；只有用户明确要求“全量验证”时，才运行最新 stage debug 脚本的 `--suite all`。日常精度修复和阶段验收优先选择典型 case 集，必须覆盖 GVA 1:2、1:3，`chunk_size=64/128`，`V=128/256`，以及变长尾块；当前 stage3 debug 典型集使用 `test_stage3_debug_golden.py --cases F1,F3,F19,F20,L9`，其中 F19 覆盖 1:2/V256/chunk64，F20 覆盖 1:3/V256/chunk64，L9 覆盖 V256/chunk128/变长尾块，F1/F3 覆盖 V128 的 chunk64/chunk128。若怀疑执行卡死，先用同一典型集加 `--kernel-only` 确认 kernel 返回；只有典型集失败或用户要求时再扩大到全量。stage debug 脚本默认隐藏逐 task 的 `ct.single` 明细，只打印失败摘要和 `compare_progress`；需要查看原始 CT 报告时再加 `--verbose-ct`。`--input-scale` 表示 `[-scale, scale]` 的半宽，默认 `1.0`。
...
```bash
# 具体 warmup/repeat/cases 环境变量以当前最新 stage 的 profile 脚本为准。
msprof \
  --output=<profile_output_dir> \
  python3 <repo_root>/fla/ops/ascendc/gdn/chunk_gdn_bwd/prepare_wy_repr_bwd/test/profile_<stage>_perf.py
```
...
- stage3 debug op 当前只写 `debug_dkbg/debug_dvb` 两个中间产物输出槽，分别解释为 `Dkb/DK`，不写 `dk/dv/dbeta/dg`，也不再写 stage0/stage1/stage2 的旧 debug 输出。为了少改临时 ABI，Python ctypes wrapper 可继续复用底层第 8/9 个 debug 输出槽作为当前 stage debug tensor，但 wrapper 名称和测试脚本必须把它们解释为 `Dkb/DK`；未写输出可使用 1 元素占位 tensor，避免大 shape 在 kernel launch 前因调试输出分配 OOM。`prepare_wy_repr_bwd_full` 等正式四输出 wrapper 仍必须使用真实 shape，不能套用该占位策略。
