...
class ChunkBwdDqkwgGenerator(CaseGenerator):
    ...
    def after_case_config(self, case_config: CaseConfig) -> CaseConfig:
        ...
        num_chunks = math.ceil(T / chunkSize) if not is_fix else (T + chunkSize - 1) // chunkSize
        case_config.inputs[H_INDEX].shape = [B, H, num_chunks, K, V]
        case_config.inputs[DH_INDEX].shape = [B, H, num_chunks, K, V]

        if not is_fix:
            num_seqs = case_config.inputs[CU_SEQLENS_INDEX].shape[0] - 1
            total_chunks = sum(
                math.ceil(case_config.inputs[CU_SEQLENS_INDEX].range_values[1] / chunkSize)
                for _ in range(num_seqs)
            )
            case_config.inputs[CHUNK_INDICES_INDEX].shape = [total_chunks, 2]
        else:
            case_config.inputs[CHUNK_INDICES_INDEX].shape = [num_chunks, 2]
